/**
  * FAILURE
  * 
  * The best effect systems help you reason about failure scenarios, and deal
  * with them appropriately. Like Rust and Go, effect systems use static types
  * to help you understand if code can fail, and how.
  * 
  * However, functional effect systems like ZIO provide a more nuanced take 
  * on error handling that recognizes the existence of both recoverable and
  * unrecoverable errors, and provides a rich set of tools for dealing with
  * both.
  */
 ...
 * TYPED EXAMPLE
 * 
 * This is a checkout example where the errors are all strongly statically typed, 
 * using Scala 3 enums for each class of error, and with clearly distinguished
    ...
    suite("FallibleSpec")(
      test("succeed") {
        ...
        /**
          * EXERCISE 1
          * 
          * Implement the `succeed` method on `Fallible` so that it returns a
          * `Fallible` that succeeds with the specified value.
          */
      ...
      test("fail") {
        ...
        /**
          * EXERCISE 2
          * 
          * Implement the `fail` method on `Fallible` so that it returns a
          * `Fallible` that fails with the specified error.
          */
      ...
      test("attempt") {
        ...
        /**
          * EXERCISE 3
          * 
          * Implement the `attempt` method on `Fallible` so that it returns a
          * `Fallible` that succeeds with the value produced by the specified
          * effect, or fails with the error produced by the effect.
          */
      ...
      test("fromEither") {
        ...
        /**
          * EXERCISE 4 
          * 
          * Implement the `fromEither` method on `Fallible` so that it returns a
          * `Fallible` that succeeds with the value produced by the specified
          * `Either`, or fails with the error produced by the `Either`.
          */
        assertTrue(result.attempt == Right(42))
      } @@ ignore,
      test("map") {
        ...
        /**
          * EXERCISE 5
          * 
          * Implement the `map` method on `Fallible` so that it returns a
          * `Fallible` that applies the specified function to the value produced
          * by the source `Fallible`.
          */
      ...
      test("flatMap") {
        ...
        /**
          * EXERCISE 6
          * 
          * Implement the `flatMap` method on `Fallible` so that it returns a
          * `Fallible` that applies the specified function to the value produced
          * by the source `Fallible`.
          */
        assertTrue(result.attempt == Right(43))
      ...
      test("get") {
        ...
        /**
          * EXERCISE 7
          * 
          * Implement the `get` method on `Fallible` so that it returns the value
          * produced by the `Fallible`, or throws an exception if the `Fallible`
          * failed.
          */
      ...
      test("orDie") {
        ...
        /**
          * EXERCISE 8
          * 
          * Implement the `orDie` method on `Fallible` so that it returns a
          * `Fallible` that succeeds with the value produced by the source
          * `Fallible`, or fails with the error produced by the source `Fallible`.
          */
      ...
      test("mapError") {
        ...
        /**
          * EXERCISE 9
          * 
          * Implement the `mapError` method on `Fallible` so that it returns a
          * `Fallible` that applies the specified function to the error produced
          * by the source `Fallible`.
          */
      ...
      test("refineOrDie") {
        ...
        /**
          * EXERCISE 10
          * 
          * Implement the `refineOrDie` method on `Fallible` so that it returns a
          * `Fallible` that succeeds with the value produced by the source `Fallible`,
          * or fails with the error produced by the source `Fallible`, refined by the
          * specified partial function (by refining, it is meant that if the error doesn't 
          * match the refined type, it will be thrown as an exception).
          */
